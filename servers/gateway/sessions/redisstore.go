...
func NewRedisStore(client *redis.Client, sessionDuration time.Duration) *RedisStore {
	...
	// return a new RedisStore struct
	return &RedisStore{
		Client:          client,
...
func (rs *RedisStore) Save(sid SessionID, sessionState interface{}) error {
	...
	marshaledSessState, err := json.Marshal(sessionState)
	if err != nil {
		return fmt.Errorf("error marshaling the session state into JSON %v\n", err)
	}

	err = rs.Client.Set(sid.getRedisKey(), marshaledSessState, rs.SessionDuration).Err()
	if err != nil {
		return fmt.Errorf("error saving session state: %v\n", err)
	}
	return nil
}
...
func (rs *RedisStore) Get(sid SessionID, sessionState interface{}) error {
	...
	if err = expire.Err(); err != nil {
		return fmt.Errorf("error setting expiration time: %v", err)
	}
...
func (rs *RedisStore) Delete(sid SessionID) error {
	...
	if err := rs.Client.Del(sid.getRedisKey()).Err(); err != nil {
		return fmt.Errorf("error deleting the data stored in redis fr the given SessionID: %v\n", err)
	}
