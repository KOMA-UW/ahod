...
exports.createGroup = (req, res, next) => {
  ...
  // }; //JSON.parse(req.get('X-User'))
  /*
   */
  if (temp_user) {
    const url = req.protocol + '://' + req.get('host');
    const imgPath =
      req.file != null ? url + '/images/' + req.file.filename : '';
        ...
        Member.findById(temp_user.id)
          .then(member => {
            if (!member) {
              ...
              group.members.push(newMember); // add new member to the group
              ...
              let index = group.members.find(memb => memb._id === member._id);
...
async function sendInvite(
  ...
  let temp_token = crypto.randomBytes(10).toString('base64');
  ...
  let url =
    'https://' + host + 'onboarding' + 'https://payinpayout.tech/onboarding';
  ...
  const inviteMessage = {
    ...
    html:
      `<strong> Hello ${email}, this is ${creatorName} from ${groupName}. <br> We want you to join our clique and save money. '\n
      ...
      .then(member => {
        ...
        Group.updateOne({ _id: req.params.id, admin: temp_user.id }, newGroup)
          ...
          .catch(error => {
            res
              .status(500)
              .json({ members: 'Updating a group details failed!' });
          });
      })
      .catch(error => {
    ...
    Group.findById(req.params.id)
      .then(group => {
        ...
        if (group.rounds.length < group.members.length) {
          // check if everyone has got a chance to win or not
          if (candidates.length > 0) {
            if (candidates.length < group.members.length) {
              // first round of the group

              let randomNumber = Math.floor(Math.random() * candidates.length);
              let index = group.rounds.find(
                memb => memb._id === candidates[randomNumber]._id
              );

              if (index === undefined) {
                // if the current winner did not win last time.
                ...
                round
                  ...
                  .catch(error => {
                    if (error) {
                      res
                        .status(500)
                        .json({ message: 'saving the round failed' });
                      return;
                    }
                  });
                ...
                group
                  ...
                  .catch(error => {
                    res
                      .status(500)
                      .json({ message: 'saving the group failed' });
                    return;
                  });
              }
          ...
          group.rounds.length === group.members.length &&
          candidates.length > 0 &&
        ...
        res.status(200).json({
          message: 'Selecting the winner success',
          winner: group.rounds.pop().winner
        });
      })
      .catch(error => {
...
exports.getGroupRounds = (req, res, next) => {
  ...
  if (temp_user) {
    Group.findById(req.params.id)
      .then(group => {
        res.status(200).json({
          message: 'Getting all rounds success',
          rounds: group.rounds
        });
      })
      .catch(error => {
